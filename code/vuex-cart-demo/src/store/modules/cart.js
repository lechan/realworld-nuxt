...
const getters = {
  ...
  checkedPrice (state) {
    return state.cartProducts.reduce((sum, prod) => {
      if (prod.isChecked) {
        sum = parseFloat((sum + prod.totalPrice).toFixed(2))
      }
      return sum
    }, 0)
  }
...
const mutations = {
  addToCart (state, product) {
    ...
    if (prod) {
      prod.count++
      prod.isChecked = true
      prod.totalPrice = parseFloat((prod.count * prod.price).toFixed(2))
    } else {
  ...
  updateProduct (state, {
    ...
    if (prod) {
      prod.count = count
      prod.totalPrice = parseFloat((count * prod.price).toFixed(2))
    }
  }
}
const actions = {}
